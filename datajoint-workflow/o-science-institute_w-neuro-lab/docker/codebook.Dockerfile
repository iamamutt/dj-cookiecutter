...
ARG DEPLOY_KEY=o-science-institute_w-neuro-lab-deploy.pem
ARG REPO_OWNER=dj-sciops
ARG REPO_NAME=o-science-institute_w-neuro-lab
...
RUN apt-get -y update &&\
	...
	cp -f /tmp/${REPO_NAME}/docker/codebook.datajoint_config.json /usr/local/bin/.datajoint_config.json &&\
...
RUN mamba env create -qv -f /tmp/${REPO_NAME}/environment.yml &&\
	conda run --no-capture-output -n neurowf pip install /tmp/${REPO_NAME}

USER root:anaconda
RUN	conda run --no-capture-output -n neurowf ipython kernel install --name=neurowf &&\
	rm -fR /tmp/${REPO_NAME} &&\
	mamba clean -ya
