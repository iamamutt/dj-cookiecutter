...
def install_package(session: nox.Session, *extras: str) -> None:
...
def write_version(session: nox.Session) -> None:
    ...
    with open("src/neurowf/version.py", "w") as out:
        session.run("echo", f'__version__ = "{version}"', stdout=out, external=True)

    git_action_bot(session, add=["src/neurowf/version.py"])
...
def pytest(session: nox.Session) -> None:
    ...
    install_package(session, "test", "sciops")
    session.run("pytest")
